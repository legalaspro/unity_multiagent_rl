...
def test_competitive_evaluator_initialization(mock_logger, competitive_eval_config_dict, mock_snapshot_fn_factory, mocker):
    cfg = Namespace(**competitive_eval_config_dict)
    
    # Mock UnityEnvWrapper to return our MockMAEnv instance
    mock_env_instance = mock_ma_env(competitive_eval_config_dict) # Get a configured MockMAEnv
    mocker.patch('evals.competitive_evaluator.UnityEnvWrapper', return_value=mock_env_instance)
    ...
    assert evaluator.env == mock_env_instance # Check if mock env was used
    ...
    assert len(evaluator.random_policy.action_spaces) == mock_env_instance.n_agents
...
def test_play_one_episode_outcome(competitive_eval_config_dict, mock_snapshot_fn_factory, mocker):
...
def test_competitive_evaluator_run_integration(mock_logger, competitive_eval_config_dict, mock_snapshot_fn_factory, mocker):
    cfg_dict = competitive_eval_config_dict
    cfg = Namespace(**cfg_dict)

    mock_env_instance = mock_ma_env(cfg_dict)
    ...
    def mock_play_one_episode_for_run2(agentA, agentB, teams, env):
        ...
        elif agentB.id == "agent_step_0": # Opponent is the first snapshot
            return 1.0 # Current agent (step 100) always beats snapshot 0
        return original_play_one_episode(agentA, agentB, teams, env) # Fallback
...
def test_competitive_evaluator_state_persistence(mock_logger, competitive_eval_config_dict, mock_snapshot_fn_factory, tmp_path, mocker):
    cfg_dict = competitive_eval_config_dict
    cfg = Namespace(**cfg_dict)
...
# TODO:
# - Test _select_opponents strategy if it becomes more complex.
# - Test edge cases like no opponents, or when eval_episodes_per_matchup is 0 or 1.
# - Test with different team configurations if the logic depends heavily on it.
# - The evaluator's interaction with UnityEnvWrapper (cfg.env_id, worker_id) is mocked away.
#   If there were specific setup based on these, that's not covered.
# - The `RandomPolicy` initialization `RandomPolicy(self.env.action_spaces)` is tested implicitly
#   by ensuring `mock_env_instance.action_spaces` is a list of gym.Space objects.
# - The current tests assume `MockEvalAgent.act` returns a list of torch tensors.
#   `play_one_episode` then does `np.vstack([a.numpy() for a in actions_A])`.
#   This means each `a` in `actions_A` should be a 1D tensor if `act_dim > 1`, or scalar tensor if `act_dim = 1`.
#   The `MockEvalAgent` in helpers samples from `gym.space` which gives numpy, then converts to tensor.
#   This should be compatible.
# - The `competitive_eval_config_dict` defines `teams` as `[[0], [1]]` for a 2-agent env.
#   The `mock_ma_env` is also set up for 2 agents. This is consistent.
#   The `num_env_agents` in `mock_snapshot_fn_factory` and `test_play_one_episode_outcome`
#   are derived from `sum(len(team) for team in cfg.teams)`. This is also consistent.
# - The `MockEvalAgent` constructor was updated to take `env_action_spaces_list` to correctly
#   mimic how `RandomPolicy` would be initialized and how `agent.act` is used in `play_one_episode`.
# - `competitive_eval_config_dict`'s `eval_episodes` is not directly used by this evaluator's `run` method.
#   It uses `self.random_episodes` (hardcoded to 16 in source) and `self.eval_episodes_per_matchup`.
#   The config key `competitive_eval_episodes` maps to `eval_episodes_per_matchup`.
#   My fixture sets `eval_episodes = 4`, this is unused. `competitive_eval_episodes = 2` is used.
#   I will remove the unused `eval_episodes` from the test config to avoid confusion.
# - The `eval_log_filename` is also not used by this `CompetitiveEvaluator` class. It uses the passed `logger`.
# - The `num_processes` from config is also not used by this class.
# - The `render` and `verbose` flags are not used.
# I'll clean up the config fixture.The tests for `CompetitiveEvaluator` have been implemented in `tests/test_competitive_evaluator.py`.
