...
def get_algo_config_spaces_env(
    ...
    **kwargs # Allow overriding any specific param
    ...
    if 'MAPPO' in kwargs.get("algo_name", ""):
        base_config.update({
            "n_rollout_threads": n_rollout_threads, "n_steps": n_steps, "num_mini_batch": n_mini_batch,
            "shared_policy": False, # Test with separate policies
    ...
    if 'MATD3' in kwargs.get("algo_name", ""):
        base_config.update({
            ...
            "policy_freq": policy_freq,        # Keep original name for compatibility
        })
...
def test_smoke_run_mappo():
    ...
    for update_idx in range(num_updates):
        for step in range(config.n_steps):
            ...
            next_obs_list_np, rewards_np, dones_np, truncs_np, info_dict = env.step(processed_actions_for_env)
...
def test_smoke_run_maddpg():
    ...
    while total_steps_done < config.batch_size + 5: # Ensure enough samples for a few batches
        ...
        next_obs_list_np, rewards_np, dones_np, truncs_np, info_dict = env.step(actions_list_np)
...
def test_smoke_run_masac():
    ...
    buffer = ReplayBuffer(
        ...
        device=torch.device(config.device), n_step=config.n_step, gamma=config.gamma
    )
    initial_params = get_initial_params(algo)
